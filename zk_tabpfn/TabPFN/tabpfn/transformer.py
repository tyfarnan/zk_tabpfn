    ...
    def forward(self, src):
        ...
        if src_mask is None:
            if self.global_att_embeddings is None:
                full_len = x_src.shape[0] + style_src.shape[0]
                if self.full_attention:
                    ...
                    src_mask = float(single_eval_pos) + float(len(style_src))
                else:
        ...
        return output[single_eval_pos+style_src.shape[0]+(self.global_att_embeddings.num_embeddings if self.global_att_embeddings else 0):]
