...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def predict_proba(self, X, normalize_with_test=False, with_onnx=True, return_logits=False):
        """
    ...
    def predict(self, X, return_winning_probability=False, normalize_with_test=False, with_onnx=True, return_logits=False):
        p = self.predict_proba(X, normalize_with_test=normalize_with_test, with_onnx=with_onnx, return_logits=return_logits)
...
def transformer_predict(model, eval_xs, eval_ys, eval_position,
    ...
    def predict(eval_xs, eval_ys, with_onnx, used_style, softmax_temperature, return_logits):
        ...
        with inference_mode_call:
            ...
            if with_onnx:
                ...
                model = GizaModel(model_path="./tabpfn.onnx")
                result = model.predict(
                    input_feed ={"src":eval_xs.cpu().numpy(), "onnx::Unsqueeze_1":eval_ys.cpu().numpy()}, verifiable=False)
...
#                return 'robust_all'
    ...
    for batch_input, batch_label in zip(inputs, labels):
        ...
        with warnings.catch_warnings():
            ...
            if device == 'cpu':
                output_batch = checkpoint(predict, batch_input, batch_label, with_onnx, style_, softmax_temperature_, True)
            # else:
